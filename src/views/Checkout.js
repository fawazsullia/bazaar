...
function Checkout() {
  ...
  const handleOrder = async () => {
    ...
    try {
      state.currentUser.cart.map(async (product) => {
        ...
        let res = await fetch(`${config.baseUrl}}/user/orders`, {
          method: "PUT",
          headers: {
    ...
    <div className={checkoutStyle.container}>
      ...
      <div className={checkoutStyle.productsContainer}>
        {state.currentUser.cart.map((product) => {
          ...
          return (
            <div className={checkoutStyle.cardContainer}>
              <span className={checkoutStyle.title}>
                {requiredProduct.title}
              </span>
              <span className={checkoutStyle.price}>
                $ {requiredProduct.price}
              </span>
              <span className={checkoutStyle.count}>{product.count}</span>
              <span className={checkoutStyle.total}>
                $ {product.count * requiredProduct.price}
              </span>
            </div>
          );
        })}
      ...
      <button
        className={checkoutStyle.placeOrder2}
        type="button"
        onClick={handleOrder}
      >
        {ordering ? "Ordering..." : "Place Order"}
