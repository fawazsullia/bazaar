...
function Profile() {
  ...
  const saveProfile = () => {
    setsaving(true);
    setedit(false);
    fetch(`${config.baseUrl}/user/edit`, {
      method: "PUT",
      headers: {
      ...
      .then((response) => {
        if (!response.status) {
          alert("Could not save at this moment");
        }
        setsaving(false);
      })
      .catch((err) => {
  ...
  const deleteUser = () => {
    ...
    firebase
      ...
      .currentUser.delete()
      .then(() => {
        fetch(`${config.baseUrl}/user/${state.currentUser.uid}`, {
          method: "DELETE",
        }).then(() => {
    ...
    <div className={profileStyle.container}>
      <div className={profileStyle.personalInfo}>
        ...
        <form>
          ...
          <input
            type="text"
            style={{ outline: edit ? "1px solid red" : "none" }}
            readOnly={!edit}
            value={displayName}
            onChange={(e) => {
      ...
      <div className={profileStyle.dangerSection}>
        ...
        <p>
          Clicking on this button will permanently delete your account. Any
